-- =============================================
-- FIX BUYER CANCELLATION NOTIFICATION
-- =============================================
-- Problem: When buyer cancels, notification says "Store cancelled your order"
-- Solution: Check which status changed first to determine who cancelled
-- =============================================
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
        -- CANCELLATION HANDLING - Check who cancelled
        -- If buyer_status changed to cancelled, buyer cancelled it
        -- If farmer_status changed to cancelled but buyer_status didn't, farmer cancelled it
        IF NEW.farmer_status = 'cancelled' AND OLD.farmer_status != 'cancelled' THEN
            -- Farmer status just changed to cancelled
            -- Check if buyer also cancelled (both statuses changed in same update)
            IF NEW.buyer_status = 'cancelled' AND OLD.buyer_status != 'cancelled' THEN
                -- BUYER CANCELLED (both changed together)
            ...
            ELSIF NEW.buyer_status = 'cancelled' AND OLD.buyer_status = 'cancelled' THEN
                -- Buyer status was already cancelled, so farmer is cancelling now
                -- This shouldn't normally happen, but handle it
                NULL; -- Do nothing
            ELSE
                -- FARMER CANCELLED (only farmer_status changed)
                PERFORM create_notification(
                    NEW.buyer_id,
                    'Order Cancelled',
            ...
            END IF;
        -- Handle other farmer status changes (not cancellation)
        ELSIF OLD.farmer_status != NEW.farmer_status THEN
...
RAISE NOTICE '✅ Buyer cancellation notification fixed!';
RAISE NOTICE '📋 Logic: Checks which status changed to determine who cancelled';
RAISE NOTICE '🔹 Buyer cancels → "You have successfully cancelled"';
RAISE NOTICE '🔹 Farmer cancels → "Store has cancelled your order"';
